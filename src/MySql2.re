...
module Error = {
  ...
  [@bs.get]
  external sql_state : Js.Exn.t => Js.Nullable.t(string) = "sqlState";
  [@bs.get]
  external sql_message : Js.Exn.t => Js.Nullable.t(string) = "sqlMessage";
  ...
  let from_js = exn => {
    ...
    Failure(
      {j|$name - $code ($errno) - $message - ($sql_state) $sql_message|j},
    );
  };
};
...
let result_select = (rows, meta) =>
  `Select((rows, Belt_Array.map(meta, decode_meta_record)));
...
external execute :
  (
    Connection.t,
    Options.t,
    (Js.Nullable.t(Js.Exn.t), Js.Json.t, array(Js.Json.t)) => unit
  ) =>
  unit =
  "execute";
